...
Bei Bildern heißt die entsprechende Float-Umgebung \texttt{figure}.
Die Positionierung kann durch Angabe von Buchstaben
\texttt{h}, \texttt{t}, \texttt{b} und \texttt{p}
...
Somit bleibt eigentlich nur noch die Verwendung der \texttt{h}-Option,
...
verschoben.
Und auch wenn die Platzierung mit der \texttt{h}-Option klappt,
...
die in der Datei \texttt{AllePfade.tex} definiert sind.
...
was hier durch die Option \texttt{width=\textbackslash linewidth} verwirklicht wurde.

Wichtig anzumerken ist, dass alle Zeilen innerhalb der \texttt{Figure}"=Umgebung
...
Mit Hilfe von Paket \texttt{Subfig} \cite{Cochran2005} können Bilder auch in
...
Zu \gls{gls:tikz} und dem darauf aufsetzenden \gls{gls:pgfplots} gibt
...
Auch hier wird mit Hilfe der Anweisung
\texttt{\bs resizebox\{\bs textwidth\}\{!\}\{...\}}
Skalierung auf die volle Seitenbreite durchgeführt.
...
\texttt{./figures-compiled/} genommen wird.
...
ein einfaches \index{Diagramm!Punkt-}Punktdiagramme (\cref{fig:ica})
